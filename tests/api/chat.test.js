...
describe('Chat API Endpoints', () => {
    ...
    beforeEach(async () => {
        ...
        const patientRes = await request(app).post('/api/v1/users/register').set('X-Forwarded-For', fakeIp).send(patientData);
        if (patientRes.body?.data) { mockPatientToken = patientRes.body.data.accessToken; }

        const therapistData = { fullName: 'Chat Therapist', email: 'chattherapist@test.com', password: 'Password123!', role: 'therapist' };
        const therapistRes = await request(app).post('/api/v1/users/register').set('X-Forwarded-For', fakeIp).send(therapistData);
        if (therapistRes.body?.data) {
    ...
    describe('GET /api/v1/chat/rooms', () => {
        it('should return 401 Unauthorized if no token is provided', async () => {
            const res = await request(app).get('/api/v1/chat/rooms').set('X-Forwarded-For', fakeIp);
            expect(res.statusCode).toBe(401);
        });

        it('should return empty chat rooms list for a new patient', async () => {
            const res = await request(app).get('/api/v1/chat/rooms').set('Authorization', `Bearer ${mockPatientToken}`).set('X-Forwarded-For', fakeIp);
    ...
    describe('GET /api/v1/chat/room/:otherUserId', () => {
        it('should allow patient to get or create chat room with a therapist', async () => {
            const res = await request(app).get(`/api/v1/chat/room/${mockTherapistId}`).set('Authorization', `Bearer ${mockPatientToken}`).set('X-Forwarded-For', fakeIp);
