...
/**
 * Helper: Invalidate therapist-related caches
 */
const invalidateTherapistCaches = async (therapistProfileId) => {
  await Promise.all([
    deleteCachePattern("therapists:list:*"),
    deleteCache(`therapist:${therapistProfileId}`),
    deleteCachePattern("recommendations:*"),
  ]);
};
...
const createTherapistProfile = asyncHandler(async (req, res) => {
  const {
    bio, isStudent, licenseNumber, specializations,
    yearsOfExperience, sessionRate, qualifications,
  ...
  await invalidateTherapistCaches(therapistProfile._id);

  return res
    .status(201)
    .json(new ApiResponse(201, createdProfile, "Therapist profile created successfully"));
});
...
const updateTherapistProfile = asyncHandler(async (req, res) => {
  const {
    bio, licenseNumber, specializations,
    yearsOfExperience, sessionRate, qualifications, supervisorId,
  } = req.body;
  ...
  if (sessionRate !== undefined) {
    if (sessionRate <= 0) throw new ApiError(400, "Session rate must be greater than 0");
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, updatedProfile, "Therapist profile updated successfully"));
});

/**
 * Get therapist by ID (cached 5 min)
...
const getTherapistById = asyncHandler(async (req, res) => {
  ...
  if (cached) {
    return res.status(200).json(new ApiResponse(200, cached, "Therapist fetched successfully (cached)"));
  }
  ...
  await setCache(cacheKey, therapist.toObject(), 300);
...
/**
 * Get all therapists with filters and pagination (cached 2 min)
 * @route GET /api/v1/therapists
 * @access Public
 */
const getAllTherapists = asyncHandler(async (req, res) => {
  const {
    specialization, minRating, maxRate, minRate,
    minExperience, verifiedOnly, sortBy, page = 1, limit = 10,
  } = req.query;

  const cacheKey = `therapists:list:${hashQuery(req.query)}`;
  const cached = await getCache(cacheKey);
  if (cached) {
    return res.status(200).json(new ApiResponse(200, cached, "Therapists fetched successfully (cached)"));
  }
  ...
  else if (sortBy === "rate-asc") sort.sessionRate = 1;
  else if (sortBy === "rate-desc") sort.sessionRate = -1;
  ...
  return res.status(200).json(
    new ApiResponse(200, data, "Therapists fetched successfully")
  );
});
...
const updateAvailability = asyncHandler(async (req, res) => {
  ...
  for (const day in availability) {
    if (!validDays.includes(day.toLowerCase())) throw new ApiError(400, `Invalid day: ${day}`);
    if (!Array.isArray(availability[day])) throw new ApiError(400, `Availability for ${day} must be an array`);

    for (const slot of availability[day]) {
      if (!slot.start || !slot.end) throw new ApiError(400, "Each time slot must have start and end time");
  ...
  const therapistProfile = await Therapist.findOne({ userId: req.user._id });
  if (!therapistProfile) throw new ApiError(404, "Therapist profile not found");
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, updatedProfile, "Availability updated successfully"));
});
...
const updateQualifications = asyncHandler(async (req, res) => {
  ...
  if (!Array.isArray(qualifications)) throw new ApiError(400, "Qualifications must be an array");
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, updatedProfile, "Qualifications updated successfully"));
});
...
const updateSpecializations = asyncHandler(async (req, res) => {
  ...
  if (!Array.isArray(specializations)) throw new ApiError(400, "Specializations must be an array");
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, updatedProfile, "Specializations updated successfully"));
});
...
const getStudentsUnderSupervisor = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, { students, count: students.length }, "Students fetched successfully"));
});
...
const verifyTherapist = asyncHandler(async (req, res) => {
  ...
  await invalidateTherapistCaches(therapist._id);

  return res
    .status(200)
    .json(new ApiResponse(200, updatedTherapist,
